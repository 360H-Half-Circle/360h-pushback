...
ParticleFilter::ParticleFilter(std::vector<std::shared_ptr<miku::Distance>> sensors, lemlib::Chassis& chassis_ref)
    : distance_sensors(sensors), chassis(chassis_ref) {
...
void distance_reset(lemlib::Chassis& chassis, std::shared_ptr<ParticleFilter> pf, Point estimate, float particle_spread) {
...
void ParticleFilter::update_previous_belief(Pose robot_speed) {
    ...
    std::cout << "Robot Speed: (" << robot_speed.x << ", " << robot_speed.y << ", " << robot_speed.theta << ")\n";
    
    chassis.setPose(
        clamp_field(chassis.getPose().x + robot_speed.x),
        clamp_field(chassis.getPose().y + robot_speed.y),
...
void ParticleFilter::update_particle_weights() {
    ...
    standard_radians robot_theta = chassis.getPose().theta;
    ...
    for(size_t i = 0; i < distance_sensors.size(); ++i) {
        float expected = get_expected_reading(
            Point(chassis.getPose().x, chassis.getPose().y),
            distance_sensors[i],
            cos_theta, 
            sin_theta).distance;
