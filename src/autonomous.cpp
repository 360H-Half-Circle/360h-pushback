...
void auton::auton_skills(lemlib::Chassis& chassis) {
    ...
    chassis.moveToPoint(-20, 17, 2000, {.forwards=true, .maxSpeed=60});;
    chassis.waitUntilDone();
    if (override_pressed()) return;
    ...
    chassis.moveToPoint(-62, 13, 5000, {.minSpeed=127});
    chassis.waitUntilDone();
    if (override_pressed()) return;
...
void auton::sawp(lemlib::Chassis& chassis) {
    ...
    chassis.moveToPose(-10, 7, 315, 500, {.forwards=false, .minSpeed=60});
    chassis.waitUntilDone();
...
void auton::seven_wing_left(lemlib::Chassis& chassis) {
    ...
    chassis.moveToPoint(-24.5, 49, 1000, {.forwards=false, .maxSpeed=90});
    chassis.waitUntilDone();
