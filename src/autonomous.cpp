...
auto imu = pros::Imu(INERTIAL_PORT);
Rotation odom(PERP_TRACKING_WHEEL);
...
void auton::auton_skills(lemlib::Chassis& chassis) {
    auto skills_start_time = std::chrono::high_resolution_clock::now();
    ...
    Task matchloaderDelay([&](){
        while (true) {
            double deltaPitch = imu.get_pitch() - pitch;
            if (deltaPitch < -3) {
                delay(200);
    ...
    // chassis.moveToPoint(-64.5, 34, 5000, {.maxSpeed=65, .minSpeed=65});
    // chassis.waitUntilDone();
    ...
    while (abs(error) > 5) {
        ...
        power = std::clamp(power, -60.0f, 60.0f);
    ...
    chassis.moveToPoint(-62, 33.5, 500, {.forwards=false, .maxSpeed=50});
    chassis.waitUntilDone();
    ...
    // chassis.turnToPoint(-20, 18, 500, {.forwards=true});
    // chassis.waitUntilDone();
    // if (override_pressed()) return;

    // chassis.moveToPoint(-20, 18, 750, {.forwards=true});;
    // chassis.waitUntilDone();
    // if (override_pressed()) return;

    chassis.moveToPoint(-33, 33.5, 1000, {.forwards=true, .maxSpeed=80});
    chassis.waitUntilDone();

    chassis.turnToPoint(-10, 13, 200, {.forwards=false});
    chassis.waitUntilDone();

    chassis.moveToPoint(-10, 13, 1000, {.forwards=false, .maxSpeed=80});
    chassis.waitUntilDone();
    if (override_pressed()) return;
    ...
    chassis.moveToPoint(-42.8, 52, 1200);
    chassis.waitUntilDone();
    if (override_pressed()) return;
    ...
    // matchload 
    chassis.moveToPoint(-52.5, 46.5, 250, {.minSpeed=90});
    chassis.waitUntilDone();
    chassis.moveToPoint(-69.5, 46.5, 1000, {.maxSpeed=40});
    chassis.waitUntilDone();
    if (override_pressed()) return;
    ...
    chassis.moveToPose(-40.5, 62, 270, 500, {.forwards=false, .lead=0.1, .minSpeed=110});
    chassis.waitUntilDone();
    if (override_pressed()) return;

    chassis.moveToPoint(30, 62, 1700, {.forwards=false, .minSpeed=60});
    // chassis.moveToPoint(55, 59, 1700, {.forwards=false, .minSpeed=60});
    chassis.waitUntilDone();
    if (override_pressed()) return;
    ...
    ///////////////////////////////////////////////
    // score
    chassis.moveToPoint(45, 50, 750, {.forwards=false});
    chassis.waitUntilDone();

    chassis.turnToHeading(90, 500);
    chassis.waitUntilDone();

    chassis.moveToPoint(26.5, 50, 300, {.forwards=false, .minSpeed=40});
    ...
    // matchload
    chassis.moveToPoint(48, 45.5, 750, {.minSpeed=90});
    chassis.waitUntilDone();
    ...
    chassis.moveToPose(64.5, 10.5, 180, 1750, {.lead=0.3, .minSpeed=50});
    chassis.waitUntilDone();
    if (override_pressed()) return;
    ...
    // chassis.moveToPoint(64.5, -40, 5000, {.maxSpeed=65, .minSpeed=65});
    // chassis.waitUntilDone();
    // if (override_pressed()) return;
    // CROSS BARRIER
    ...
    chassis.turnToPoint(16, 14, 500, {.forwards=true});
    chassis.waitUntilDone();
    if (override_pressed()) return;

    chassis.moveToPoint(16, 14, 750, {.forwards=true});;
    chassis.waitUntilDone();
    if (override_pressed()) return;

    chassis.turnToPoint(6, 9, 500, {.forwards=true});
    chassis.waitUntilDone();
    if (override_pressed()) return;

    chassis.moveToPoint(6, 9, 750, {.forwards=true, .maxSpeed=35});;
    chassis.waitUntilDone();
    if (override_pressed()) return;
    ...
    //--------------LOW GOAL----------------//
    ///////////////////////////////////////////////
    intake.bottom_intake.move_velocity(250);
    ...
    chassis.turnToPoint(37.8, -46, 500, {.forwards=false});
    chassis.waitUntilDone();
    if (override_pressed()) return;

    chassis.moveToPoint(37.8, -46, 1400, {.forwards=false, .minSpeed=30});
    delay(800);
    ...
    chassis.moveToPose(40.5, -58, 0, 2000, {.forwards=false, .lead=0.01, .minSpeed=110});
    chassis.waitUntilDone();
    if (override_pressed()) return;

    chassis.moveToPoint(-30, -62, 1500, {.forwards=false, .minSpeed=70});
    // chassis.moveToPoint(-55.5, -58, 1700, {.forwards=false, .minSpeed=70});
    chassis.waitUntilDone();
    if (override_pressed()) return;
    ...
    chassis.moveToPoint(-24.5, -48, 400, {.forwards=false, .minSpeed=40});
    ...
    // matchload
    chassis.moveToPoint(-50, -46, 500, {.minSpeed=90});
    chassis.waitUntilDone();
    ...
    chassis.moveToPoint(24.5, -48.5, 1800, {.forwards=false, .maxSpeed=60});
    chassis.waitUntilDone();
    if (override_pressed()) return;
    ...
    chassis.moveToPose(-64, -12.5, 0, 1750, {.lead=0.3, .minSpeed=120});
    chassis.waitUntilDone();
    if (override_pressed()) return;
    ...
    auto parkStart = std::chrono::high_resolution_clock::now();
    double p = imu.get_pitch();
    Task pc([&](){
        while (true) {
            double deltaPitch = imu.get_pitch() - p;
            auto current_time = std::chrono::high_resolution_clock::now();
            std::chrono::duration<float> elapsed = current_time - parkStart;
                
            if (elapsed.count() * 1000.0f <= 750) {
                continue;
            }
...
void auton::four_wing_right(lemlib::Chassis& chassis) {
    ...
    chassis.moveToPose(-28.5, -48, 320, 1000, {.forwards=false, .lead=0.1, .minSpeed=80, .earlyExitRange=2});
    chassis.waitUntilDone();

    // chassis.swingToHeading(270, DriveSide::LEFT, 500, {.direction=AngularDirection::CW_CLOCKWISE, .minSpeed=90});

    chassis.tank(-127, -30, true);
    delay(500);
    chassis.tank(0, 0, true);
...
void auton::sawp(lemlib::Chassis& chassis) {
    ...
    chassis.turnToPoint(-46.5, 46.5, 300);
    chassis.waitUntilDone();

    chassis.moveToPoint(-46.5, 46.5, 1000);
    chassis.waitUntilDone();
    ...
    chassis.moveToPoint(-24.5, 46.5, 500, {.forwards=false, .minSpeed=70});
    delay(400);
    hood.set_value(false);
    chassis.waitUntilDone();
    ...
    chassis.waitUntilDone();
    chassis.moveToPoint(-75.5, 46.5, 650, {.maxSpeed=40});
    chassis.waitUntilDone();

    resetRobotPos(chassis, right_dist, "positive_y");

    chassis.moveToPose(-14, 9, 315, 1400, {.forwards=false, .lead=0.1, .minSpeed=30, .earlyExitRange=1.5});
    delay(1200);
    hoodBottom.set_value(true);
    midgoal.set_value(false);
    chassis.waitUntilDone();
...
void auton::seven_split_left(lemlib::Chassis& chassis) {
    ...
    chassis.moveToPoint(-14, 9, 1500, {.forwards=false, .minSpeed=30});
    chassis.waitUntilDone();
