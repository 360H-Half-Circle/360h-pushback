...
double getAveragedSensorReading(pros::Distance& sensor) { 
    ...
    for (int i = 0; i < NUM_SAMPLES; ++i) { // NUM_SAMPLES is 10 by definition
        ...
        if (reading != PROS_ERR && reading > 0 && reading <= MAX_VALID_DISTANCE_MM) { 
...
void resetRobotPos(lemlib::Chassis& chassis, pros::Distance& sensor, const std::string& wallAxisDirection) {
    ...
    if (sensor.get_port() == FRONT_DISTANCE_SENSOR) {
        sensorOffset = FRONT_SENSOR_OFFSET;
    } else if (sensor.get_port() == BACK_DISTANCE_SENSOR) {
        sensorOffset = BACK_SENSOR_OFFSET;
    } else if (sensor.get_port() == LEFT_DISTANCE_SENSOR) {
        sensorOffset = LEFT_SENSOR_OFFSET;
    } else if (sensor.get_port() == RIGHT_DISTANCE_SENSOR) {
